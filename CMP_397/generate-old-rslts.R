...
# to need to focus on [CO2], [CH4], global temps & all forcing components


# 0. Set Up --------------------------------------------------------------------
# need to install the specific version of hector being used the ‘3.1.1’ release
...
my_run_hector <- function(ini){
    ...
    run(hc)
    vars <- c(GLOBAL_TAS(), GMST(), CONCENTRATIONS_CH4(), CONCENTRATIONS_CO2(), rf_vars)
